...
const CheckoutPage = () => {
  ...
  const discountedAmount = selectedCoupon
    ? totalAmount - (totalAmount * selectedCoupon.discount) / 100
    : totalAmount;
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      const courseIds = courses.map((c) => c._id);
      const response = await initiateCheckout(courseIds, discountedAmount, "razorpay");
      const order = response?.order;
  ...
  const handleWalletPayment = async () => {
    ...
    if (!wallet || wallet.balance < discountedAmount) {
      toast.error("Insufficient wallet balance. Please use Razorpay or recharge your wallet.");
      return;
    }
    ...
    try {
      const courseIds = courses.map((c) => c._id);
      await initiateCheckout(courseIds, discountedAmount, "wallet");
  ...
  const handleRemove = async (courseId: string, courseName: string) => {
    ...
    try {
      await removeFromCart(courseId);
      setCourses((prev) => prev.filter((c) => c._id !== courseId));
      setSelectedCoupon(null); // Reset coupon if cart changes
      toast.info(`${courseName} removed from cart.`);
    } catch (error: any) {
      const errorMessage = error?.response?.data?.message;
      if (errorMessage?.includes("already enrolled")) {
  ...
  const canProceedWithWallet = wallet && wallet.balance >= discountedAmount;
        ...
        <>
          ...
          <div className="bg-white border rounded-lg p-6 mb-6">
            ...
            <div
              ...
              onClick={() => setAvailableCoupons(availableCoupons.length > 0 ? [] : availableCoupons)} // Toggle visibility
            >
              <div className="flex items-center justify-between">
            ...
            <div className="flex flex-col gap-4 sm:flex-row">
              ...
              >
                {isWalletProcessing ? (
                  ...
                  `💰 Pay via Wallet (₹${discountedAmount.toLocaleString()})`
                )}
              </button>
              ...
              >
                {isRazorpayProcessing ? (
                  ...
                  `💳 Pay via Razorpay (₹${discountedAmount.toLocaleString()})`
                )}
              </button>
            </div>
          </div>
          ...
          <div className="flex justify-between items-center">
            ...
            >
              {isProcessing ? (
                ...
                `Pay ₹${discountedAmount.toLocaleString()}`
              )}
            </button>
          </div>
        </>
