...
import LearningPathCard from "../../../components/StudentComponents/LearningPathCard";
import { getAllCategories, GetLMSCourses } from "../../../api/action/StudentAction";
import type { LearningPath, LearningPathFilterResponse, Category } from "../interface/studentInterface";
  ...
  const fetchPaths = async () => {
    try {
      ...
      if (response.success) {
        ...
        setTotalPages(response.total ? Math.ceil(response.total / pathsPerPage) : 1);
      } else {
        ...
        <main className="w-full lg:w-3/4">
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {[...Array(pathsPerPage)].map((_, index) => (
                <div
                  key={index}
