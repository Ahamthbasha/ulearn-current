...
const InstructorDashboard = () => {
  ...
  const [reportRange, setReportRange] = useState<"daily" | "weekly" | "monthly" | "yearly" | "custom">("daily");
  ...
  const fetchReport = async (page: number = 1) => {
    if (reportRange === "custom" && (!startDate || !endDate || new Date(startDate) > new Date(endDate))) {
    ...
    try {
      ...
      const response = await getRevenueDashboard(reportRange, page, limit, startDate, endDate);
  ...
  const categoryData = categorySales.map((item) => ({
    ...
    percentage: Math.round((item.totalSales / totalCourseSales) * 100) || 0,
  }));
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-4 sm:mb-6 md:mb-8">
          ...
          <div className="bg-white rounded-lg sm:rounded-xl shadow-sm p-3 sm:p-4 md:p-6 border border-gray-100 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs sm:text-sm font-medium text-gray-600">Total Course Sales</p>
                <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">{totalCourseSales}</p>
              </div>
          ...
          <div className="bg-white rounded-lg sm:rounded-xl shadow-sm p-3 sm:p-4 md:p-6 border border-gray-100 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs sm:text-sm font-medium text-gray-600">Published Courses</p>
                <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">{publishedCourses}</p>
              </div>
          ...
          <div className="bg-white rounded-lg sm:rounded-xl shadow-sm p-3 sm:p-4 md:p-6 border border-gray-100 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs sm:text-sm font-medium text-gray-600">Categories Used</p>
                <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">{categoryWiseCount}</p>
              </div>
...
>
  ...
  <option value="custom">Custom</option>
            ...
            <button
              ...
              disabled={reportLoading || (reportRange === "custom" && (!startDate || !endDate))}
              className={`${
                reportLoading || (reportRange === "custom" && (!startDate || !endDate))
          ...
          {reportData.length > 0 ? (
            <div className="mt-2 sm:mt-4 md:mt-6 overflow-x-auto">
              ...
              <div className="flex justify-end gap-2 sm:gap-4 mb-2 sm:mb-4">
                <button
                  onClick={() => exportRevenueReport(reportRange, "excel", startDate, endDate)}
                  className="bg-blue-600 text-white px-2 sm:px-4 py-1 sm:py-2 rounded hover:bg-blue-700 transition w-full sm:w-auto"
                >
                  Export Excel
                </button>
                <button
                  onClick={() => exportRevenueReport(reportRange, "pdf", startDate, endDate)}
              ...
              <table className="w-full border rounded shadow-sm">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th className="px-2 sm:px-4 py-1 sm:py-2 text-left text-xs sm:text-sm">Coupon</th>
                    <th className="px-2 sm:px-4 py-1 sm:py-2 text-left text-xs sm:text-sm">Total Amount</th>
                    <th className="px-2 sm:px-4 py-1 sm:py-2 text-left text-xs sm:text-sm">Your Revenue</th>
                  </tr>
                </thead>
                <tbody>
                  {reportData.map((item, idx) => (
                    <tr key={idx} className="border-t">
                      <td className="px-2 sm:px-4 py-1 sm:py-2 text-xs sm:text-sm">#{item.orderId.slice(-7)}</td>
                      <td className="px-2 sm:px-4 py-1 sm:py-2 text-xs sm:text-sm">{item.date}</td>
                      ...
                      <td className="px-2 sm:px-4 py-1 sm:py-2 text-xs sm:text-sm">
                        {item.couponCode !== "N/A" ? (
                          <>
                            {item.couponCode} ({item.couponDiscount}%)<br />
                            <span className="text-red-600">-₹{(item.couponDiscountAmount || 0).toFixed(2)}</span>
                          </>
                        ) : (
                          "N/A"
                        )}
                      ...
                      <td className="px-2 sm:px-4 py-1 sm:py-2 text-xs sm:text-sm text-green-700 font-semibold">
                ...
                <tfoot>
                  <tr className="bg-gray-100 font-semibold">
                    <td colSpan={5} className="px-2 sm:px-4 py-1 sm:py-2 text-right text-xs sm:text-sm">
                      Total Instructor Revenue
                    </td>
                    <td className="px-2 sm:px-4 py-1 sm:py-2 text-xs sm:text-sm text-green-700">
                      ₹{reportData.reduce((sum, item) => sum + item.instructorRevenue, 0).toFixed(2)}
                    </td>
                  </tr>
                </tfoot>
              </table>
            ...
            <p className="text-gray-500 mt-2 sm:mt-4 text-sm sm:text-base">
              {reportLoading ? "Loading report..." : "No data available. Please generate a report."}
            </p>
          )}
        </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center space-x-1 mt-0.5 sm:mt-1">
                        <span className="text-xs sm:text-sm text-gray-500">{course.count} sales</span>
                      </div>
                    </div>
                    <div className="text-right">
