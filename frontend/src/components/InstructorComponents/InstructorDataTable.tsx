...
import { type InstructorDataTableProps } from "./interface/instructorComponentInterface";

const InstructorDataTable = <T extends Record<string, unknown>>({
  data,
  columns,
  loading = false,
  error = null,
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                data.map((record, index) => (
                  <tr key={getRecordKey(record, index)} className="hover:bg-gray-50">
                    {columns.map((col) => {
                      ...
                      return (
                        ...
                        >
                          {col.render
                            ? col.render(cellValue as T[keyof T], record, index)
                            : <span className="text-sm text-gray-900">{String(cellValue ?? '')}</span>}
                        </td>
                      );
                    })}
                    {actions.length > 0 && (
