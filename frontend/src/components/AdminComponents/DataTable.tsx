            ...
            {loading ? (
              ...
              <div className="divide-y divide-gray-200">
                {data.map((record, index) => (
                  <div key={'id' in record && typeof record.id === 'string' ? record.id : index} className="p-4 space-y-3">
                    {columns.map((column) => {
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (
                  ...
                  data.map((record, index) => (
                    <tr key={'id' in record && typeof record.id === 'string' ? record.id : index} className="hover:bg-gray-50 transition-colors">
                      {visibleColumns.map((column) => (
                        <td
                          key={String(column.key)}
                        ...
                        >
                          {column.render ? (
                            ...
                            <span>{record[column.key] as React.ReactNode}</span>
                          )}
                        </td>
                      ))}
                      {actions.length > 0 && (
