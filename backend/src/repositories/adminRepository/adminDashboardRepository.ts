...
import { InstructorDocument, CourseDetails,ILearningPathOrderDetails,IOrder } from "../../interface/adminInterface/IadminInterface";
...
export class AdminDashboardRepository implements IAdminDashboardRepository {
  ...
  const orders = (await this._orderRepo.findSuccessfulOrdersLean()) as IOrder[];
  let totalAdminShare = 0;

  for (const order of orders) {
    const totalNoOfCoursesInAnOrder =
      order.courses.length +
      order.learningPaths.reduce((sum: number, lp: ILearningPathOrderDetails) => sum + lp.courses.length, 0);
...
> {
  ...
  for (const order of orders) {
    ...
    monthlyTotals[key] = (monthlyTotals[key] || 0) + Number(orderAdminShare.toFixed(2));
  }
...
async getCourseSalesReportFiltered(
  ...
  for (const order of orders) {
    ...
    const totalAdminShare = Number(courses.reduce(
  ...
  async getTopSellingCourses(
    ...
    const pipeline: PipelineStage[] = [
      ...
      {
        $project: {
          allCourses: { $concatArrays: ["$standaloneCourses", "$learningPathCourses"] },
        },
      },
      { $unwind: "$allCourses" },
      {
 ...
 async getTopSellingCategories(
  ...
  const pipeline: PipelineStage[] = [
    { $match: { status: "SUCCESS" } },
    {
      $facet: {
        standaloneCourses: [
          ...
          { $unwind: { path: "$categoryDetails", preserveNullAndEmptyArrays: true } }, // Allow for missing categories
          {
            $group: {
              _id: "$courseDetails.category",
              categoryName: { $first: { $ifNull: ["$categoryDetails.categoryName", "Unknown"] } }, // Use categoryName from Category collection
              salesCount: { $sum: 1 },
            },
          },
        ],
    ...
    {
      $project: {
        allCategories: { $concatArrays: ["$standaloneCourses", "$learningPathCourses"] },
      },
    },
  ...
  return await this._orderRepo.performAggregation<{ categoryName: string; salesCount: number }>(
    pipeline
  );
}
}