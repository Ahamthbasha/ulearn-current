...
export class StudentLmsEnrollmentRepo implements IStudentLmsEnrollmentRepo {
  ...
  async getEnrolledLearningPaths(
  ...
  > {
    try {
      ...
      const result = await Promise.all(
        enrollments
          .filter(
            (
              enrollment,
            ): enrollment is ILearningPathEnrollment & { learningPathId: ILearningPath } =>
          ...
          .map(async (enrollment) => {
            ...
            const noOfHours = learningPath.items?.reduce((total, item) => {
            ...
            if (learningPath.items?.length) {
              const courseIds = learningPath.items.map(item =>
                item.courseId instanceof Types.ObjectId ? item.courseId : item.courseId._id
              );
              ...
              const totalCompletion = courseEnrollments.reduce((sum, percentage) => sum + percentage, 0);
            ...
            const order = orders.find(o =>
              o.learningPaths.some(lp => lp.learningPathId.equals(learningPath._id))
            );
      ...
      throw new Error(`Failed to fetch enrolled learning paths: ${(error as Error).message}`);
    }
  }
...
async getLearningPathDetails(
  ...
  try {
    ...
    const coursesWithPresignedUrls = await Promise.all(
      (learningPath.courses || []).map(async (course) => {
        ...
        if (courseEnrollment) {
          if (courseEnrollment.certificateGenerated && courseEnrollment.certificateUrl) {
            certificateUrl = await getPresignedUrl(courseEnrollment.certificateUrl);
          }
        ...
        return {
          ...
          thumbnailUrl: presignedThumbnailUrl,
          price: coursePrice,
          effectivePrice: courseOrder?.offerPrice ?? course.effectivePrice ?? coursePrice,
          certificateUrl,
    ...
    return {
      ...
      enrollment,
      courses: coursesWithPresignedUrls as Array<ICourse & { certificateUrl?: string; completionPercentage?: number }>,
    };
  } catch (error) {
    throw new Error(`Failed to fetch learning path details: ${(error as Error).message}`);
  }
}
  ...
  async updateUnlockedOrder(
    ...
    try {
      ...
      throw new Error(`Failed to update unlocked order: ${(error as Error).message}`);
    }
  }
  ...
  async markCourseCompleted(
    ...
    try {
      const enrollment = await this._learningPathEnrollmentRepo.findById(enrollmentId.toString());
      if (!enrollment) {
        throw new Error("Enrollment not found");
      }
      const learningPath = await this._learningPathRepo.findById(enrollment.learningPathId.toString());
      if (!learningPath) {
        throw new Error("Learning path not found");
      }
      ...
      const populatedEnrollment = await this._enrollmentRepository.findByUserAndCourseWithPopulate(
        (enrollment.userId as Types.ObjectId).toString(),
        (courseId as Types.ObjectId).toString(),
        [
          {
      ...
      const course = populatedEnrollment.courseId as unknown as IPopulatedCourse;
      const allChaptersCompleted =
        course.chapters?.length > 0 &&
        course.chapters.every((chapter: IChapter) =>
          populatedEnrollment.completedChapters.some(
            (cc: ICompletedChapter) => cc.chapterId.equals(chapter._id) && cc.isCompleted,
          ),
        );
        ...
        course.quizzes?.length > 0
          ? course.quizzes.every((quiz: IQuiz) =>
              populatedEnrollment.completedQuizzes.some(
                (cq: ICompletedQuiz) => cq.quizId.equals(quiz._id) && cq.scorePercentage >= 50,
              ),
            )
          : true;
      if (!allChaptersCompleted || !allQuizzesCompleted) {
        throw new Error("Not all chapters or quizzes are completed for this course");
      }
      // Mark course as completed in LearningPathEnrollment
      const completedCourse = enrollment.completedCourses.find((cc) => cc.courseId.equals(courseId));
      if (completedCourse) {
      ...
      const completedCourses = enrollment.completedCourses.filter((cc) => cc.isCompleted).length;
      if (completedCourses === totalCourses) {
        enrollment.completionStatus = "COMPLETED";
      } else if (completedCourses > 0) {
        enrollment.completionStatus = "IN_PROGRESS";
      }
      if (enrollment.completionStatus === "COMPLETED" && !enrollment.certificateGenerated) {
        const student = await this._studentRepository.findById(enrollment.userId);
        if (!student || !student.username) {
          throw new Error("Student or student username not found");
        }
        const instructor = await this._instructorRepository.findById(learningPath.instructorId.toString());
      ...
      const updatedEnrollment = await this._learningPathEnrollmentRepo.updateOne(
        { _id: enrollmentId },
        {
      ...
      throw new Error(`Failed to mark course as completed: ${(error as Error).message}`);
    }
  }
