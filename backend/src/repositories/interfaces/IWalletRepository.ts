...
export interface IWalletRepository {
  ...
  createWallet(ownerId: Types.ObjectId, onModel: string, role: string): Promise<IWallet>;
  creditWallet(ownerId: Types.ObjectId, amount: number, description: string, txnId: string): Promise<IWallet | null>;
  debitWallet(ownerId: Types.ObjectId, amount: number, description: string, txnId: string): Promise<IWallet | null>;
