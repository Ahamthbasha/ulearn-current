...
process.on('uncaughtException', (error: Error) => {
  appLogger.error('Uncaught Exception', { message: error.message, stack: error.stack });
  process.exit(1);
});
...
const start = async () => {
  try {
    ...
    httpServer.listen(port, () => {
      ...
      appLogger.info(`HTTP request logging enabled (${process.env.NODE_ENV || 'development'} mode)`);
    });
