...
export async function generateMembershipReceiptPdf(order: IInstructorMembershipOrder): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    // Company details
    doc.fillColor(textColor).fontSize(20).font('Helvetica-Bold')
       .text('uLearn', 110, 35);
    ...
    doc.fillColor(primaryColor).fontSize(24).font('Helvetica-Bold')
       .text('MEMBERSHIP RECEIPT', 0, 35, { align: 'right', width: doc.page.width - 40 });
    
    // Status badge
    const statusColor = order.paymentStatus === 'paid' ? successColor : '#ef4444';
    ...
    doc.fontSize(10).font('Helvetica')
       ...
       .text(new Date(order.createdAt).toLocaleDateString('en-IN'), 220, infoBarY + 25)
    ...
    doc.font('Helvetica')
       ...
       .text(new Date(order.createdAt).toLocaleDateString('en-IN'), 130, currentY + 44);
    ...
    const detailsData = [
      ...
      { label: 'Start Date', value: new Date(order.startDate).toLocaleDateString('en-IN') },
      { label: 'End Date', value: new Date(order.endDate).toLocaleDateString('en-IN') },
      { label: 'Description', value: plan.description || 'Standard membership plan' }
    ];
    ...
    // Summary box
    doc.rect(40, currentY, doc.page.width - 80, 60).fill('#f8fafc').stroke('#e5e7eb');
    ...
    // Divider line
    doc.moveTo(40, footerStartY).lineTo(doc.page.width - 40, footerStartY).stroke('#e5e7eb');
    ...
    doc.fillColor(textColor).fontSize(8).font('Helvetica')
       .text('This is a computer-generated receipt. For any queries, please contact support.', 0, footerStartY + 35, { 
         align: 'center', 
         width: doc.page.width 
       });
