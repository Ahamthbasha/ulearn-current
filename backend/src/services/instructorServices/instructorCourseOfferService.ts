...
export class InstructorCourseOfferService
  implements IInstructorCourseOfferService
{
  ...
  constructor(
    ...
    courseOfferRepo: IInstructorCourseOfferRepo,
  ) {
  ...
  async createCourseOffer(
    ...
    const existingOffer = await this._courseOfferRepo.findOne({
      courseId,
      instructorId,
      status: { $ne: "rejected" },
    });
  ...
  async editCourseOffer(
    ...
    if (existingOffer.instructorId.toString() !== instructorId)
      throw new Error("Unauthorized");
  ...
  async resubmitOffer(
    ...
    if (offer.instructorId.toString() !== instructorId)
      throw new Error("Unauthorized");

    const updatedOffer = await this.editCourseOffer(
      instructorId,
      offerId,
      discountPercentage,
      startDate,
  ...
  async getOffersByInstructor(
    ...
    const result = await this._courseOfferRepo.getOffersByInstructor(
      instructorId,
      page,
      limit,
      search,
  ...
  async deleteCourseOffer(
    instructorId: string,
    offerId: string,
  ): Promise<void> {
    const deletedOffer = await this._courseOfferRepo.deleteById(
      offerId,
      instructorId,
    );
  ...
  async getInstructorCourseOfferById(
    offerId: string,
    instructorId: string,
