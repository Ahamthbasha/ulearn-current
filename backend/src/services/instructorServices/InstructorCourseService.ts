...
export class InstructorCourseService implements IInstructorCourseService {
  ...
  async updateCourse(courseId: string, courseData: Partial<ICourse>): Promise<CourseResponseDto | null> {
  ...
  async getInstructorCoursesPaginated(
    ...
    const result = await this._courseRepository.getCoursesByInstructorWithPagination(
      instructorId, 
      page, 
      limit, 
      search
  ...
  async isCourseAlreadyCreatedByInstructor(courseName: string, instructorId: string): Promise<boolean> {
    const existing = await this._courseRepository.findCourseByNameForInstructor(courseName, instructorId);
    return !!existing;
  }

  async isCourseAlreadyCreatedByInstructorExcluding(courseName: string, instructorId: string, courseId: string): Promise<boolean> {
    const existing = await this._courseRepository.findCourseByNameForInstructorExcludingId(courseName, instructorId, courseId);
    return !!existing;
  }
  ...
  async publishCourse(courseId: string): Promise<ICourse | null> {
    return await this._courseRepository.updateCourse(courseId, { isPublished: true });
  }
}