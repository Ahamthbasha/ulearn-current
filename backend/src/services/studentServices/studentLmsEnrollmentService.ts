...
import { LearningPathDTO, LearningPathDetailsDTO } from "../../dto/userDTO/lmsEnrollDTO";
import { mapToLearningPathDTO, mapToLearningPathDetailsDTO } from "../../mappers/userMapper/lmsEnrollMapper";
import { getPresignedUrl } from "../../utils/getPresignedUrl";
import { IOrderRepository } from "../../repositories/interfaces/IOrderRepository";

export class StudentLmsEnrollmentService implements IStudentLmsEnrollmentService {
  ...
  async getEnrolledLearningPaths(userId: string): Promise<LearningPathDTO[]> {
    try {
      ...
      const enrolledData = await this._studentLmsEnrollmentRepo.getEnrolledLearningPaths(userObjectId);
      const learningPaths = await Promise.all(
        enrolledData.map(async ({ learningPath, enrollment }) =>
          await mapToLearningPathDTO(learningPath, enrollment, this._orderRepository, userObjectId),
        ),
      );
  ...
  async getLearningPathDetails(
    ...
    try {
      ...
      const { learningPath, enrollment, courses } = await this._studentLmsEnrollmentRepo.getLearningPathDetails(
        userObjectId,
        learningPathObjectId,
      );
      return await mapToLearningPathDetailsDTO(learningPath, enrollment, courses, this._orderRepository, userObjectId, getPresignedUrl);
    } catch (error) {
      throw new Error(`Service error: ${(error as Error).message}`);
    }
  }
  ...
  async completeCourseAndUnlockNext(
    ...
    try {
      ...
      const courseDetails = await this._studentLmsEnrollmentRepo.getLearningPathDetails(userObjectId, learningPathObjectId);
      const targetCourse = courseDetails.courses.find((c) => c._id.equals(courseObjectId));
      if (!targetCourse) {
        throw new Error("Course not found in learning path");
      }

      const learningPathEnrollment = await this._studentLmsEnrollmentRepo.markCourseCompleted(
      ...
      const nextItem = learningPath.items.find((item) => item.order === nextOrder);
  ...
  async generateLearningPathCertificate(
    ...
    try {
      const userObjectId = new Types.ObjectId(userId);
      const learningPathObjectId = new Types.ObjectId(learningPathId);
      const enrollment = await this._studentLmsEnrollmentRepo.getLearningPathDetails(userObjectId, learningPathObjectId);
      if (!enrollment) {
        throw new Error("Learning path enrollment not found");
      }
      const certificateUrl = await this._studentLmsEnrollmentRepo.generateLearningPathCertificate(
