...
export class StudentLmsEnrollmentController
  implements IStudentLmsEnrollmentController
{
    ...
    try {
      ...
      if (!userId) {
        res
          ...
          .json({ error: STUDENT_ERROR_MESSAGE.STUDENT_UNAUTHORIZED });
        return;
      }
    ...
    try {
      ...
      if (!userId || !learningPathId) {
        res.status(StatusCode.BAD_REQUEST).json({
          error: STUDENT_ERROR_MESSAGE.USERID_LEARNINGPATHID_REQUIRED,
        });
        return;
      }

      const details = await this.service.getLearningPathDetails(
        userId,
        learningPathId,
      );
  ...
  async completeCourseAndUnlockNext(
    ...
    try {
      ...
      if (!userId || !learningPathId || !courseId) {
        res.status(StatusCode.BAD_REQUEST).json({
          error: STUDENT_ERROR_MESSAGE.USER_LEARNINGPATH_COURSE_IDS_REQUIRED,
        });
        return;
      }

      const enrollment = await this.service.completeCourseAndUnlockNext(
        userId,
        learningPathId,
        courseId,
      );
      res.status(StatusCode.OK).json({
        data: enrollment,
        message: STUDENT_SUCCESS_MESSAGE.COURSE_COMPLETED_NEXT_COURSE_UNLOCKED,
      });
    } catch (error) {
      const errorMessage = (error as Error).message;
      appLogger.error("error in complete course and unlock", error);

      if (
        errorMessage.includes(
          STUDENT_ERROR_MESSAGE.ALL_CHAPTERS_QUIZES_NEED_TO_COMPLETED,
        )
      ) {
        res.status(StatusCode.BAD_REQUEST).json({
          message:
            STUDENT_ERROR_MESSAGE.COMPLETE_ENTIRE_COURSE_CHAPTERS_AND_QUIZES,
        });
      } else {
    ...
    try {
      ...
      if (
        ...
        res
          ...
          .json({ error: STUDENT_ERROR_MESSAGE.CERTIFICATE_NOT_AVAILABLE });
        return;
      }
