    ...
    try {
      ...
      if (!userId) {
        res
          ...
          .json({ message: StudentErrorMessages.STUDENT_UNAUTHORIZED });
        return;
      }

      const dashboardData =
        await this._dashboardService.getStudentDashboardData(userId);
      const monthlyPerformance =
        await this._dashboardService.getMonthlyPerformance(userId);
  ...
  async exportCourseReport(
    ...
    try {
      ...
      if (!userId) {
        res
          .status(StatusCode.UNAUTHORIZED)
          .json({ message: StudentErrorMessages.NOT_FOUND_STUDENT });
        return;
      }
  ...
  async exportSlotReport(
    ...
    try {
      ...
      if (exportFormat === "pdf") {
        ...
        res.setHeader(
          "Content-Disposition",
          "attachment; filename=slot_report.pdf",
        );
        ...
        res.setHeader(
          "Content-Disposition",
          "attachment; filename=slot_report.xlsx",
        );
