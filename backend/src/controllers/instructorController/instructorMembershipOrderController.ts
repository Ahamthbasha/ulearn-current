    ...
    try {
      ...
      if (!instructorId) {
        res.status(StatusCode.UNAUTHORIZED).json({
          success: false,
          message: ResponseMessages.INSTRUCTOR_NOT_FOUND,
        });
        return;
      }
...
async getInstructorOrders(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    res.status(StatusCode.INTERNAL_SERVER_ERROR).json({ message: "Failed to fetch purchase history" });
  }
}
...
async downloadReceipt(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    if (!txnId || !instructorId) {
      res.status(400).json({ message: "Missing txnId or user not authenticated" });
      return;
    }
