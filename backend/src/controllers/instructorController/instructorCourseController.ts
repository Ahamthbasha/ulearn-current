...
export class InstructorCourseController implements IInstructorCourseController {
  ...
  async publishCourse(
    ...
    try {
      ...
      if (publishDate) {
        // Parse the date - it should be in ISO format from frontend
        ...
        console.log('  - Parsed (UTC):', parsedPublishDate.toISOString());
        console.log('  - Parsed (IST):', parsedPublishDate.toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }));
        ...
        if (parsedPublishDate <= now) {
          console.log('  - Validation failed: publishDate is in the past');
          res.status(StatusCode.BAD_REQUEST).json({
            success: false,
            message: INSTRUCTOR_ERROR_MESSAGE.INVALID_PUBLISH_DATE,
          });
