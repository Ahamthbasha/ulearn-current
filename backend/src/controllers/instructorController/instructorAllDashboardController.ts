  ...
  implements IInstructorAllDashboardController
{
  ...
  async getDashboard(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      if (!instructorId)
        throwAppError(
          UnauthorizedError,
          INSTRUCTOR_ERROR_MESSAGE.INSTRUCTOR_UNAUTHORIZED,
        );
      ...
      const topCoursesWithUrls = await Promise.all(
        data.topCourses.map(
          async (
            c: ITopSellingCourse,
          ): Promise<ITopSellingCourse & { thumbnailUrl: string }> => ({
            ...c,
            thumbnailUrl: c.thumbnailUrl, // Direct Cloudinary URL
          }),
        ),
      );
  ...
  async getDetailedRevenueReport(
    ...
    try {
      ...
      if (
        !range ||
        !allowedRanges.includes(range as (typeof allowedRanges)[number])
      ) {
      ...
      if (isNaN(pageNum) || isNaN(limitNum) || pageNum < 1 || limitNum < 1) {
        throwAppError(
          BadRequestError,
          INSTRUCTOR_ERROR_MESSAGE.INVALID_PAGE_LIMIT,
        );
      }
  ...
  async exportRevenueReport(
    ...
    try {
      ...
      if (
        !format ||
        !allowedFormats.includes(format as (typeof allowedFormats)[number])
      ) {
      ...
      const reportData: ReportData[] = result.data.map(
        (item: IRevenueReportItem): ReportData => ({
