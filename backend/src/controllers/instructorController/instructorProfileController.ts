    ...
    try {
      ...
      if (!decoded) {
        res
          ...
          .json({ message: InstructorErrorMessages.ACCESS_DENIED });
        return;
      }

      const instructorProfile = await this._profileService.getProfile(decoded);
  ...
  async updateProfile(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      if (file) {
        ...
        profilePicUrl = await uploadToCloudinary(file, "instructors");
      }
